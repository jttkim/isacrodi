...
public class SvmNodeFeatureVectorMapper implements FeatureVectorMapper<svm_node[]>, Serializable
{
  ...
  // FIXME: main method for testing -- should be made into a unit test?
  public static void main(String[] args) throws Exception
  {
    SvmNodeFeatureVectorMapper sfvm = new SvmNodeFeatureVectorMapper();
    sfvm.addComponentMapper(new NumericComponentMapper("temperature", 0, 1, 23.4));
    sfvm.addComponentMapper(new NumericComponentMapper("altitude", 2, 3, 471.1));
    CategoricalComponentMapper m = new CategoricalComponentMapper("leafcondition", 5);
    m.addState("normal", 6);
    m.addState("crinkled", 7);
    m.addState("rotten", 8);
    m.addState("yellowish", 9);
    // sfvm.parseFile(args[0]);
    svm_node[] node = sfvm.map(new FeatureVector());
    for (svm_node n : node)
    {
